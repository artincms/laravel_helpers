...
function dataTablesGrid(selector, var_grid_name, url, columns, more_data, initComplete, scrollX, scrollY, scrollCollapse, orderBy, orderByDesc, row_select, fixedColumns, start_item) {
    ...
    var dataTableOptionObj =
        {
            ...
            rowCallback: function (row, data, dataIndex) {
                if (row_select) {
                    var md5_data = md5(JSON.stringify(data));
                    if (typeof(window[var_grid_name + '_rows_selected'][md5_data]) !== 'undefined') {
    ...
    if (row_select) {
        $(document).on('click', '.' + select_one_class, function (e) {
            ...
            // Get row data
            var data = JSON.stringify(window[var_grid_name].row($row).data());
            var md5_data = md5(data);
            // Determine whether row data is in the list of selected row datas
            var index =  typeof(window[var_grid_name + '_rows_selected'][md5_data]) !== 'undefined';
            // If checkbox is checked and row ID is not in list of selected row IDs
            if (this.checked && !index) {
                window[var_grid_name + '_rows_selected'][md5_data] = data;
                // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
            } else if (!this.checked && index) {
                delete window[var_grid_name + '_rows_selected'][md5_data];
            }
            if (this.checked) {
                $row.addClass('selected');
                if (fixedColumns) {
